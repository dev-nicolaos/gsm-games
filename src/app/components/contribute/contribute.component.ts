...
export class ContributeComponent implements OnInit {
  ...
  onSubmit(info) {
    if (info.noEvents) {
      this.message = `Please include at least one event for '${info.data.gameName}'.
                      If none of the options are applicable, please fill in 'other'.`;
    } else {
      if (info.success) {
        if (info.operation === 'new') {
          this.message = `Your new game '${info.data.gameName}' has been submitted.`;
        } else {
        ...
        if (info.operation === 'new') {
          this.message = `Unfortunately, your new game '${info.data.gameName}' could not be submitted due to a technical error.`;
        } else {
